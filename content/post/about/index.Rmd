...
resources:
- name: featuredImage
  src: "fantasy_montecarlo_cover.jpg"
  params:
    description: "MIA vs NEP"
---
...
resp <- readRDS("league_matchups.rds")
```
...
rosters.json <- readRDS("rosters_json.rds")
```
...
players.points <- readRDS("players_points.rds") %>% 
...
```{r echo=FALSE}
players.ids <- readRDS("nfl_players_id.rds")
```
