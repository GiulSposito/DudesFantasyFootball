...
sim1 <- readRDS(glue("./data/simulation_v5_week{w1}_final.rds"))
sim2 <- readRDS(glue("./data/simulation_v5_week{w2}_final.rds"))
...
sim2$players_stats %>% 
  ...
  group_by(teamId) %>% 
  summarise( balance = sum(weekPts, na.rm = T)) %>% 
  inner_join(select(sim1$teams, teamId, name)) %>% 
  select(teamId, name, balance) %>% 
