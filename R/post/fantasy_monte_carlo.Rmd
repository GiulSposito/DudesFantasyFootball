...
# function to generate the team pontuation
...
matchups.points %>% 
  mutate(
    home.roster = map(home.roster, simTeam, .n=n),
    away.roster = map(away.roster, simTeam, .n=n),
    home.sim    = map(home.roster, simTeamPoints),
    away.sim    = map(away.roster, simTeamPoints)
  ) %>% 
  mutate( 
    home.win    = map2(home.sim, away.sim, function(.x,.y) (.x > .y) ),
    away.win    = map(home.win, function(.x) (!.x)),
    home.win.prob = map(home.win, mean, na.rm = T),
    away.win.prob = map(away.win, mean, na.rm = T)
  ) %>%
  mutate(
