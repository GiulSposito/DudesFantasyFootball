...
.scrapYahooPosition <- function(.pos, .size, .cookies){

  scrp_data <- seq(0,.size,25) %>%
    map_df(function(.i, .pos, .cookies){

      resp <- GET(
        url = glue("https://football.fantasysports.yahoo.com/f1/1196449/players?status=ALL&pos={.pos}&cut_type=9&stat1=S_PW_5&myteam=0&sort=AR&sdir=1&count={.i}"),
...
scrp_df <- tibble(
  ...
  split(1:nrow(.)) %>%
  map_df(function(.position, .cookies){
    .scrapYahooPosition(.position$pos, .position$size, .cookies = .cookies)
  }, .cookies=api_cookies) %>%
