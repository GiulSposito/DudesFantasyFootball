...
projections <- readRDS("../../data/week9_players_projections.rds") %>% 
...
readRDS("../../data/players_points.rds") %>%
  ...
  select( id, week, pts) %>% 
  group_by( id ) %>% 
  mutate( pts.mean = mean(pts, na.rm = T),
...
plotProjections <- function(.proj, .pos, .title, .subtitle, .slice=32) {
  ...
  nu_y <- max(proj_data$ceiling, na.rm = T) / 6
  ...
  proj_data %>% 
    ...
    geom_text(aes(x=reorder(pos_rank,desc(pos_rank)), 
              y=r_pos, 
              color=fantasy.team, 
              label=full_name), nudge_y = nu_y, nudge_x = 0.0, size=1.9) +
