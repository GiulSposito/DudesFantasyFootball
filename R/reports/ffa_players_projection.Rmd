...
readRDS("../../data/players_points.rds") %>%
  ...
  select( id, week, pts) %>% 
  group_by( id ) %>% 
  mutate( pts.med = median(pts, na.rm = T),
...
plotProjections <- function(.proj, .pos, .title, .subtitle, .slice=32) {
  ...
  nu_y <- max(proj_data$ceiling, na.rm = T) / 12
  ...
  proj_data %>% 
    ggplot(aes(group=fantasy.team)) +
      ...
      geom_text(aes(x=reorder(pos_rank,desc(pos_rank)), 
                y=r_pos, 
                color=fantasy.team, 
                label=full_name), nudge_y = nu_y, nudge_x = 0.0, size=2.9) +
...
plotProjections(projections, "QB", "Quaterbacks", "week 8", 20)
...
plotProjections(projections, "WR", "Wide Receivers", "week 8",50)
...
plotProjections(projections, "RB", "Running Backs", "week 8",50)
...
plotProjections(projections, "TE", "Tight Ends", "week 8",20)
...
plotProjections(projections, "K", "Kickers", "week 8",20)
...
plotProjections(projections, "DST", "Defenses", "week 8",32)
