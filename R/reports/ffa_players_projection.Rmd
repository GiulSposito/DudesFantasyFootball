...
p.stats %>% 
  ...
  ggplot(aes(x=sd, y=mean, color=fantasy.team)) +
  geom_point() +
  geom_text_repel(aes(label=fantasy.team), show.legend = F, size=3.5, point.padding = 1) +
  geom_label( aes(label=mean), size = 3.5, label.padding = unit(0.15, "lines"), show.legend = F) +
  theme_minimal() +
