---
title: "Dudes Football League - week 11 - Simulation"
output: 
...
sim <- readRDS("../../data/week11_simulation_v3.rds") 
...
plotPlayersDistribution <- function(.roster, .slotLevels=slotLevels, .posLevels=posLevels){
  .roster %>% 
    ...
    arrange(rosterSlot, position) %>% 
    mutate(display.order=1:nrow(.)) %>% 
    select(id, name, display.order, position, pts.range) %>% 
    mutate( pts.range = map(pts.range, base::sample, size=200, replace=T) ) %>%
    unnest(pts.range) %>% 
    ggplot(aes(x=pts.range, y=reorder(name, -display.order))) +
    #geom_violin(aes(y=pts.range, x=name, fill=position), alpha=.5) +
    stat_intervalh(.width = c(seq(.05,.95,.1))) +
    scale_color_brewer()+
    geom_point(alpha=.1) +
    theme_minimal() +
